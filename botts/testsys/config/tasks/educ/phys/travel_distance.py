...
TASK = Task(
    ...
    statement=Statement(
        ...
        Функция `travel_distance(alpha, v)` должна возвращать x-координату точки, в которой он приземлится. 
        Модуль `math` уже импортирован, также вы можете использовать в решении переменную `g`, 
        в нее автоматически будет подставлено значения 9.81.
        ''')
    ),
