...
TASK = Task(
    ...
    statement=Statement(
        ...
        Для данных параметров `x` (величина), `mu` (матожидание) и `sigma` (отклонение) посчитайте 
        и верните из функции `normal_cdf(x, mu, sigma)` значение функции распределения (CDF) 
        нормального случайного распределения с такими параметрами.
        
        *Примечание*: Вам может помочь гугл и википедия. Модуль `math` уже импортирован.
        ''')
    ),
