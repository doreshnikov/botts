...
TASK = Task(
    ...
    statement=Statement(
        ...
        Что именно должна делать функция, не сообщается. Примеров тоже не будет. Угадайте, что 
        должна возвращать функция, по вердиктам, которые сообщаются вам после каждой отправки 
        решения на тестирование.
        
        Более того, на функцию наложено определенное ограничение. Его тоже надо угадать и 
        выполнить, ориентируясь на вердикты и сообщения о проверке.
        ''')
    ),
    include=[],
